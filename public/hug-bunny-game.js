document.addEventListener("DOMContentLoaded", () => {
  ...
  function playMusic() {
    ...
    bgMusic.loop = true;  // Đảm bảo nhạc lặp lại
    bgMusic.play().catch(error => console.log("Tự động phát bị chặn:", error));
  }

  // Phát nhạc khi người chơi nhấp chuột hoặc chạm vào màn hình
  ...
  // Tạm dừng nhạc khi cửa sổ không active
  window.addEventListener("blur", () => {
    bgMusic.pause();
  });

  // Tiếp tục nhạc khi cửa sổ active trở lại
  ...
  function init() {  
    ...
    const noWall = actor => {
      ...
      if ([...settings.bunnies.filter(el => el.id !== actor.id), 
    ...
    // Thử phát nhạc khi game khởi tạo
    playMusic();
  }
